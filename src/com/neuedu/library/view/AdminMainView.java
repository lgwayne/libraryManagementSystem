...
public class AdminMainView extends JFrame{
	...
	/** 图书管理功能按钮 */
	private JButton btn__manage_book;
	
	/** 查询借阅记录功能按钮 */
	private JButton btn_quy_record;
		
	/** 用户管理按钮 */
	private JButton btn_ManageUser;
	...
	private void init()
	{
		//初始化组件
				...
				panel_right = new JPanel(new GridLayout(8, 1, 0, 30));

				label_welcome = new JLabel(
						"欢    迎  管  理    员   "+this.user.getUserName()+"   使   用   图   书   借   阅   管   理   系   统");

				Icon icon = new ImageIcon("config\\images\\manager.jpg");
				label_img = new JLabel(icon);
				label_img.setBounds(0, 0, 680, 600);

				btn__manage_book = new JButton("图书管理");
				btn_quy_record = new JButton("借阅记录查询");
				btn_ManageUser=new JButton("用户管理");
				...
				panel_right.add(new JLabel());
				panel_right.add(btn__manage_book);
				panel_right.add(btn_quy_record);
				panel_right.add(btn_ManageUser);
	...
	{
		btn__manage_book.addActionListener(new ActionListener() {
			//
			@Override
			public void actionPerformed(ActionEvent e) {
				System.out.println("U FOOL OPEN ANOTHER WINDOW");
				//显示图书查询这个嵌入式窗体
				AdminManageBookView qbv=new AdminManageBookView();
				panel_left.add(qbv);
				qbv.toFront();
			}
		});
		...
		btn_quy_record.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				AdminQueryRecordView arv=new AdminQueryRecordView();
				panel_left.add(arv);
				arv.toFront();
			}
		});
		btn_ManageUser.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				AdminManegeUserView amv=new AdminManegeUserView();
				panel_left.add(amv);
				amv.toFront();
